...
+ [2017-07-21 Fri]
  - Last day of Summer internship program.
  - Presented poster of the work done during the internship.
----------------------------------------------------------------
** Week 3 [2017-07-24 Mon] till [2017-07-28 Fri]
+ [2017-07-24 Mon] -> [2017-07-25 Tue]
  - Working on webscraping using xpath. This will help us in creating functions for DSL later.
  - Facing issues
    - If we use Python, there are many packages available, but I faced problems in scraping the websites where content is loaded using javascript ajax calls.
    - Xpath generated by inbuilt javascript functions which will be used on frontend is different from the one used in Python, also the format of xpath is slightly different for every package.
    - But we are familiar with Python and flask framework.
    - To tackle such problem, I started exploring possibilities in MEAN stack.
    - I am able to use libraries - `PhantomJS` and `CasperJS` for successfully scraping websites.
    - CasperJS is a layer on top of PhantonJS, with bare PhantomJS I was not able to open websites on which scraping is not allowed.
-------------------------------------------------------
* Month of August
** summary
- After exploring MEAN stack and PhantomJS for webscraping in headless browser. Finally I settled with Headless Chrome browser.
- Not using PhantomJS because it has a lot of issues in setting up with proxy.
- Also it is less frequently mantained as compared to Chrome.
- Made a small application in python which with Selenium webdrivers to run chrome.
- Also developed a small API in flask which let the sss-maker and sss-renderer interact with python app.
- Apart from this, I developed a JS library to perform different operations like (Add, delete, processing) on a DOM.
- This library is add to the website when it is opened in headless browser. JS library acts as a tool to perform all the tasks as mentioned in documents by Sai sir.
- Me and Sadhana started to integrate our work (mine SSS-Renderer and her's SSS-Maker) and deploy it online. But there were some issues in that. After debugging her's and mine code, we found that Headless chrome was the culprit.
- Running headless chrome in local machine is not an issue but to run it inside a container requires some other libraries to be added like 'python view'.
- While troubleshooting this I came accross similar problem asked on stackoverflow and where I found that the best solution to this is PhantomJS but ;( it never worked for us, because it is not able to recognize the proxy settings.
- For now we have kept it aside as low prioprity and I am working on DSL.
- Later on in this month, we moved to the DSL. Goal is develop a DSL which let the user perform these operations on the DOM. This DSL will help in unifying the interface of SSS project into a easy to use language.
- I discovered ANTLR4 to be very helpful in designing an interpreter.
- After several discussions with Sai sir, we both (Sai and me) figured out a potential grammar for SSS.

-------------------------------------------------------
* Month of September
** Week 1
  - Read ANTLR4 book and implement code examples provided in it.
  - Started developing a grammar for our own language.
  - Most of the time spent in making the grammar more robust and decreasing the lookahead problem.
  - This problem occured because we generialized the grammar too much. This leaves the Parser predicting the semantics of given statement. To predict the correct semantics it has to traverse through multiple routes in the syntax tree.
** Week 2 -> Week 3
  - Developed a working prototype of DSL.
  - Integrated the prototype with SSS-Renderer to create a webpage.
  - Faced issues in refining the DSL to add more flexibility in user input.
  - Have started implementing support for macros in DSL.
** Summary
  - Interpreter part of DSL successfully done.
  - Started working on implementation of preprocessor.
  - Spent week 4 in implementing preprocessor.
  - Later due to difficulties in implementation, I left that work and parked aside for implementing it later.

* Month of October
** Summary
  - The major task which was assigned in this month was to develop an IDE for DSL.
  - Explored different approaches.
    - Kanvas IDE (converts ANTLR grammar files into Kotlin based IDE)
    - Textmate Language files (tmLanguage files for syntax highlighting in Atom, VSCode and Sublime)
    - DSL Forge (converts ANTLR grammar files into web IDE based on Java and Ace editor)
    - DSL Forge editor
      - Our grammar files are written in ANTLR4 but DSL forge requires ANTLR3 grammar. So I converted the files in ANTLR3 but after everything went smooth, the webeditor was not working properly due to issues in auto-generated javascript code.
      - Tried to fix the issues but it consumed lot of time and didn't work.
    - Kanvas IDE
      - Kanvas IDE is written in Kotlin. I am new to Kotlin language. It took some time to setup the environment in eclipse.
      - I think that it will work but for now I have parked it aside. I am searching for some code snippets and methods which are discussed in the Kanvas documentation.
    - Textmate Language
      - Textmate language is a JSON and XML like language for defining an AST. This language is used in VS code, Atom, Sublime for syntax highlihgting. But it alone is not sufficient to for adding syntax highlighting to these editors. With this there are different manifest and code snippet samples are also required. 
      - Moreover, tmLanguage has a long documentation and many keywords. I am learning it and probably will do it later.
  - Presently, I have started working on DSL enhancement for supporting full SSS system which includes =modification=, =annotation=, =reassembling= and =styling=. Currently, it only supports =modification= component.
  
* Month of November
** Summary
    - In the first week of this month, I was working on DSL enhancement, which was about the integration of the M.A.R.S. components of the SSS systems. 
    - Later, this task was transferred to Sadhna.
    - Knowledge transfer to Sadhna is also in progress.
    - In the current task, I am developing a program for defining ontologies using ADT(graphs).
    - For building a graph, I found several libraries in Python.
      - I chose NetworkX for development purpose, because it gives flexibility to create objects as nodes and generate graphs and vizualizations in different formats and styles.    
      - NetworkX is widely used and lots of support material is available online.
    - I have not made much progress so far.
    - I am also reconsidering the option of creating the graphs from scratch because the kind of features which are required are not available in the NetworkX API. I am still exploring the API and also trying to work around the algorithm for meeting the requirements.

* Month of December [Week 1 & 2]
** Summary
  - In the last month of internship, I spent most of the days in knowledge transfer about SSS-DSL.
  - I was also involved in the integration of the SSS-Framework with DSL and other components of M.A.R.S..
  - Second week was spent in writing project report and discussing narratives.
